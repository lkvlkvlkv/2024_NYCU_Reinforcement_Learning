...
public:
	...
	/**
	 * get all possible next board and probability of given after state
	 
	 */
	std::vector<std::pair<board, float>> next(const board& b) const {
		std::vector<std::pair<board, float>> next;
		...
		static std::vector<std::pair<int, float>> tiles = { {1, 0.9}, {2, 0.1} };
		for (auto &space : spaces) {
			for (auto [tile, prob] : tiles) {
				board next_board = b;
				next_board.set(space, tile);
				next.emplace_back(next_board, prob / spaces.size());
			}
		}
		return next;
	}
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				float value = 0;
				std::vector<std::pair<board, float>> all_possible_state = next(move->after_state());
				for (auto [next, prob] : all_possible_state) {
					float v = estimate(next);
					value += prob * v;
				}
				move->set_value(value);

				if (move->value() + move->reward() > best->value() + best->reward())
					best = move;
			} else {
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		// TODO
		// for (auto it = path.begin(); it + 2 != path.end(); it++) {
		// 	auto s0 = it;
		// 	auto s1 = it + 1;
		// 	float target = s0->reward() + estimate(s1->after_state());
		// 	float error = target - s0->value();
		// 	s0->set_value(update(s0->before_state(), alpha * error));
		// }
		path.pop_back();
		float target = 0;
		for (auto it = path.rbegin(); it != path.rend(); it++) {
			auto s = it;
			float error = target + s->reward() - estimate(s->before_state());
			target =  update(s->before_state(), alpha * error);
		}
	}
